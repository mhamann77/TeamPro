...
export default function TeamCard({ team, isDashboard = false }) {
  ...
  const mockPlayerCount = team.memberCount || Math.floor(Math.random() * 10) + 10;
  const mockNextGame = team.nextEvent || `Mar ${Math.floor(Math.random() * 15) + 15}`;
  const mockRecord = `${Math.floor(Math.random() * 8) + 3}-${Math.floor(Math.random() * 5) + 1}-${Math.floor(Math.random() * 3)}`;

  // Mock player avatars
  const mockPlayers = [
    'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=256&h=256',
    'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=256&h=256',
    'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=256&h=256',
  ];
